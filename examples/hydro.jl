...
function solve_hydro()
    ...
    solve(m,                # Solve the model using the SDDP algorithm
        forward_passes=10,  # number of realisations in bound simulation
        backward_passes=1   # number of cutting iterations before convergence check
    )

    results = simulate(m,   # Simulate the policy
        10,                 # number of monte carlo realisations
